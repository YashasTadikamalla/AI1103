...
\begin{align}
	...
	\label{eq:26.2}
\end{align}
As the desired outcome $X_{1}<X_{2}$, is of the form
\begin{align}
    \tag{26.3}
    X_{1}\leq n-1,X_{2}=n
\end{align}
the required probability $Pr(X_{1}<X_{2})$ is given by
\begin{align}
    \tag{26.4}
    \sum_{n=1}^{n=6}p_{X_{2}}(n) \times Pr(X_{1}\leq n-1/X_{2}=n)
    \label{eq:probeqn}
\end{align}
Since $X_{1},X_{2}$ are independent, 
\begin{align}
\tag{26.5}
    Pr(X_{1}\leq n-1/X_{2}=n)=Pr(X_{1}\leq n-1) 
\end{align}
Therefore, \eqref{eq:probeqn} simplifies to
\begin{align}
    \tag{26.6}
    Pr(X_{1}<X_{2})=\sum_{n=1}^{n=6}p_{X_{2}}(n) \times F_{X_{1}}(n-1)
\end{align}
Substituting \eqref{eq:pmf} and \eqref{eq:26.2}, we get
\begin{align}
    \tag{26.7}
    Pr(X_{1}<X_{2})=\sum_{n=1}^{n=6}\dfrac{1}{6}\times\dfrac{n-1}{6} 
\end{align}
On solving, we get
\begin{align}
    \tag{26.8}
